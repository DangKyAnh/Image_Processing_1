...
Anh_V = Image.new(Anh_.mode, Anh_.size)
Anh_HSV = Image.new(Anh_.mode, Anh_.size)
...
for x in range(cao):
    for y in range(rong):
        ...
        Anh_V.putpixel((x,y), (int(V), int(V), int(V)))
        Anh_HSV.putpixel((x,y), (int(H), int(S), int(V)))


Anh_H_ = np.array(Anh_H)
Anh_S_ = np.array(Anh_S)
Anh_V_ = np.array(Anh_V)
Anh_HSV_ = np.array(Anh_HSV)

cv2.imshow('Anh Goc', Anh)
cv2.imshow('Anh H', Anh_H_)
cv2.imshow('Anh S', Anh_S_)
cv2.imshow('Anh I', Anh_V_)
cv2.imshow('Anh HSV', Anh_HSV_)
